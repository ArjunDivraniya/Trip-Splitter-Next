/**
 * Trip Expense Settlement Algorithm - Test Suite
 * 
 * This file contains test cases to validate the settlement calculation algorithm.
 * Run with: node --loader ts-node/esm test-settlement-algorithm.ts
 */
 ...
 */
function calculateSettlements(expenses: Expense[], allMembers: string[]): {
  ...
  const netBalancesRupees: Record<string, number> = {};
...
console.log("Settlements:", test4.settlements);
console.log("Expected approximately: Jagjeet→Mohil:828.83, Jagjeet→Arjun:99.33");
const expectedSum = test4.netBalances.Arjun + test4.netBalances.Jagjeet + test4.netBalances.Mohil;
console.log(`Balance sum: ${expectedSum.toFixed(2)} (should be 0.00)`);
